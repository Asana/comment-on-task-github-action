  ...
  try {
    ...
    if (context.eventName === "issue_comment") {
      /*Construct The Comment as: 
        ...
        hello world!
        Comment URL -> git.com */
      const dynamicCommentText = context.payload.comment?.user.login === "github-actions" ? `${context.payload.comment?.user.login} commented -> ${context.payload.comment?.html_url}`:`${context.payload.comment?.user.login} commented:\n\n${context.payload.comment?.body}\n\nComment URL -> ${context.payload.comment?.html_url}`;
